...
public class WebdriverUtils {
	...
	public static WebDriver openBrowser() throws Exception {
		...
		try {
			...
			System.out.println("Env value: " + System.getenv("Browse1r"));
			System.out.println("Properties File: " + Utils.getProperty("Browser"));
			...
			case FIREFOX:
				System.setProperty("webdriver.gecko.DriverFactory.getInstance().getDriver()", Constant.vFirefoxBrowserPath);

				DriverFactory.getInstance().getDriver() = new FirefoxDriver();
				break;

			case CHROME:
				ChromeOptions options = new ChromeOptions();
				System.setProperty("webdriver.chrome.DriverFactory.getInstance().getDriver()", Constant.vChromeBrowserPath);
				options.addArguments("--test-type");
				options.addArguments("--start-maximized");
				options.addArguments("--lang=en");
				options.addArguments("disable-infobars");
				DriverFactory.getInstance().getDriver() = new ChromeDriver(options);
				break;

			case IE10:
				capabilities = DesiredCapabilities.internetExplorer();
				capabilities.setCapability(CapabilityType.VERSION, "11");
				System.setProperty("webdriver.ie.DriverFactory.getInstance().getDriver()", Constant.vIEBrowserPath);
				DriverFactory.getInstance().getDriver() = new InternetExplorerDriver(capabilities);
				break;

			case SAFARI:
				DriverFactory.getInstance().getDriver() = new SafariDriver();
				break;
			...
			case SAUCELABS:
				...
				DriverFactory.getInstance().getDriver() = new RemoteWebDriver(new URL("http://" + "moises_siles" + ":"
						+ "ae252335-76d5-4b4b-9720-271f3927a9d5" + "@ondemand.saucelabs.com:80/wd/hub"), caps);
				sessionId = (((RemoteWebDriver) DriverFactory.getInstance().getDriver()).getSessionId()).toString();
				break;
		...
		System.out.println("Window height: " + DriverFactory.getInstance().getDriver().manage().window().getSize().getHeight());
		System.out.println("Window width: " + DriverFactory.getInstance().getDriver().manage().window().getSize().getWidth());
		Thread.sleep(2000);
		//Dimension d=new Dimension(1374, 788);
		//Dimension d=new Dimension(1022, 788);
	...
	public static void Deletecookie(String Name, int value, String Domain) throws Exception {
		try {
			Cookie cookie = new Cookie.Builder(Name, String.valueOf(value))
					.domain(Domain)
					.build();

			DriverFactory.getInstance().getDriver().manage().deleteCookie(cookie);
	...
	public static void Createcookie(String Name, int value, String Domain) throws Exception {
		try {
			Cookie cookie = new Cookie.Builder(Name, String.valueOf(value))
					.domain(Domain)
					.build();
	...
	public static void validatedropdownoptions(WebElement element, String[] str) throws Exception {
		try {
	...
	public static void jswait(WebDriver driver)throws Exception
	{
		try
		{
...
//	}
	public static void HoverOnElement(WebElement wElement) throws Exception {
	    try {
	        Actions builder = new Actions(DriverFactory.getInstance().getDriver());
	        builder.moveToElement(wElement).build().perform();
	    } catch (Exception ex) {
	        captureScreenShot(DriverFactory.getInstance().getDriver(), new Exception().getStackTrace()[0].getMethodName());
	        Log.error(ex.getMessage());
	        throw ex;
...
//	}
	public static void sendKeys(WebElement wElement, String text) throws Exception {
	    ...
	    if (wElement == null) {
	        Log.error("WebElement is null. Cannot perform sendKeys operation.");
	        return;
	    }
	...
	public static void goToURL(String url) throws Exception {
		try {
			...
			/*String cookieURL=WebdriverUtils.getProperty("testURL");
			DriverFactory.getInstance().getDriver().get(cookieURL);
			Thread.sleep(2000);
			WebdriverUtils.pauseTime(2000);
			System.out.println(cookieURL);
			Log.info("Going to " + cookieURL, true);*/
			...
			WebdriverUtils.pauseTime(1000);
			System.out.println(url);
			Log.info("Going to " + url, true);
			//WebdriverUtils.refreshPage();
			try{WebdriverUtils.waitForElementToBeClickable(LoginObjects.cookie(),"cookie element");
			//commented WebdriverUtils.esc();
			Thread.sleep(1000);
			WebdriverUtils.clickAction(LoginObjects.cookie());
			System.out.println("Cookie element is clicked");
			}
			catch(Exception e) {
			...
			Thread.sleep(4000);
			//commented WebdriverUtils.esc();
			/*if (WebdriverUtils.doesElementExist(By.xpath(LoginObjects.popup()))){
				WebdriverUtils.clickAction(DriverFactory.getInstance().getDriver().findElement(By.xpath(LoginObjects.popup())));
			}
			if (WebdriverUtils.doesElementExist(By.xpath(HomePageObjects.promo()))){
				WebdriverUtils.clickAction(DriverFactory.getInstance().getDriver().findElement(By.xpath(HomePageObjects.promo())));
			}
			if (WebdriverUtils.doesElementExist(By.xpath(BLPObjects.cookiebutton()))){
				WebdriverUtils.clickAction(DriverFactory.getInstance().getDriver().findElement(By.xpath(BLPObjects.cookiebutton())));
			}
			if (WebdriverUtils.doesElementExist(By.xpath(LoginObjects.signuppopup()))){
				WebdriverUtils.clickAction(DriverFactory.getInstance().getDriver().findElement(By.xpath(LoginObjects.signuppopup())));
			}*/
		} catch (Exception ex) {
			captureScreenShot(DriverFactory.getInstance().getDriver(), new Exception().getStackTrace()[0].getMethodName());
		...
		/*WebdriverUtils.waitForElementToBeClickable(LoginObjects.jblink(), "JBLink");
		WebdriverUtils.scrollToElement(LoginObjects.jblink());
		WebdriverUtils.clickAction(LoginObjects.jblink());*/
		BrowserUtils.waitFor(LoginObjects.LogInLink(), 10);
		//DriverFactory.getInstance().getDriver().navigate().refresh();
		Thread.sleep(2000);
		/*WebdriverUtils.waitForElementToBeClickable(LoginObjects.cookie(),"cookie element");
		try {
			WebdriverUtils.clickAction(LoginObjects.freeGroundShipping());
			Thread.sleep(2000);
		}
		catch (Exception e){
			System.out.println("Free ground shipping promo is not displaying.");
		}
		Thread.sleep(2000);
		WebdriverUtils.clickAction(LoginObjects.cookie());
		DriverFactory.getInstance().getDriver().navigate().refresh();*/
		...
		//WebdriverUtils.waitForElementToBepresent(By.xpath("//*[contains(@id,'ju_iframe')]"));
		Thread.sleep(3000);
		DriverFactory.getInstance().getDriver().navigate().refresh();
		Thread.sleep(3000);*/
		...
		if (WebdriverUtils.doesElementExist(By.xpath(BLPObjects.cookiebutton()))){
			WebdriverUtils.clickAction(DriverFactory.getInstance().getDriver().findElement(By.xpath(BLPObjects.cookiebutton())));
			System.out.println("Clicked on cookie element.");
		}
		}
		catch (Exception e) {
			// TODO: handle exception
			System.out.println("Cookie is not displaying.");
		}
		
		try {
			if(WebdriverUtils.doesElementExist(By.xpath(HomePageObjects.bubbleWrapperDiscount()))) {
			WebdriverUtils.clickAction(DriverFactory.getInstance().getDriver().findElement(By.xpath(HomePageObjects.closeBubbleWrapper())));
			Thread.sleep(2000);
			System.out.println("Closed Discuount Promo dialog.");
			}
		}
		catch (Exception e){
			System.out.println("Discount promo is not displaying.");
		}
		
		try {
			if(WebdriverUtils.doesElementExist(By.xpath(HomePageObjects.discountPopUp()))) {
			WebdriverUtils.clickAction(DriverFactory.getInstance().getDriver().findElement(By.xpath(HomePageObjects.closeBubbleWrapper())));
			Thread.sleep(2000);
			System.out.println("Closed Discuount Promo dialog.");
			}
	...
	public static Boolean switchBetweenWindowsForTitle(String Title) throws Exception {
		try {
            Set<String> listOfWindows = DriverFactory.getInstance().getDriver().getWindowHandles();
            System.out.println("Number of open windows: " + listOfWindows.size());

            String currentWindow = DriverFactory.getInstance().getDriver().getWindowHandle();
            Iterator<String> iterator = listOfWindows.iterator();

            while (iterator.hasNext()) {
                String selectedWindowHandle = iterator.next();
                if (!selectedWindowHandle.equals(currentWindow)) {
                    System.out.println("Searching For Windows");
                    DriverFactory.getInstance().getDriver().switchTo().window(selectedWindowHandle);
                    System.out.println("Switched to Window");
                    System.out.println("Title of the current window: " + DriverFactory.getInstance().getDriver().getTitle());

                    if (DriverFactory.getInstance().getDriver().getTitle().contains(Title)) {
                        System.out.println("Found the window with title containing '" + Title + "'");
                        return true;
                    }
                }
            }
	...
	public static Boolean switchToiframe(WebElement frame) throws Exception {
		try {
			...
			captureScreenShot(DriverFactory.getInstance().getDriver(), new Exception().getStackTrace()[0].getMethodName());
			Log.error("Error while switching to frame: " + ex.getMessage());
			return false;
		}
	...
	public static int getColumnIndex(String gridName, String columnHeader) throws Exception {
		String jsCode = "";
		try {
			jsCode = " var columnHeader = '" + columnHeader + "'; " + " var mygrid = $('" + gridName + "').grid; "
					+ " for (var i = 0; i < mygrid.getColumnsNum(); i++) {  "
					+ " var colLabel = mygrid.getColLabel(i).trim();  "
					+ " if( colLabel.toUpperCase() == columnHeader.toUpperCase() ) { " + " return i;  " + " }  "
					+ " }  " + " return -1;";
			int columnIndex = ((Long) ((JavascriptExecutor) DriverFactory.getInstance().getDriver()).executeScript(jsCode)).intValue();
			if (columnIndex < 0) {
			...
			captureScreenShot(DriverFactory.getInstance().getDriver(), new Exception().getStackTrace()[0].getMethodName());
			Log.error(
					"Class WebdriverUtils | Method getColumnIndex | Exception occurred while executing javascript. Code: "
							+ jsCode + "   Exception: " + ex.getMessage());
			throw ex;
		}
	}
	...
	public static WebElement getTxtFilter(String gridName, String columnHeader) throws Exception {
		...
		try {
			...
			WebElement filter = (WebElement) ((JavascriptExecutor) DriverFactory.getInstance().getDriver()).executeScript(jsCode);
			if (filter == null) {
				throw new Exception(
						"Filter textbox is not found. Grid: " + gridName + ", ColumnHeader: " + columnHeader);
			}
			return filter;
		} catch (Exception ex) {
			captureScreenShot(DriverFactory.getInstance().getDriver(), new Exception().getStackTrace()[0].getMethodName());
			Log.error(
					"Class WebdriverUtils | Method getTxtFilter | Exception occurred while executing javascript. Code: "
							+ jsCode + "   Exception: " + ex.getMessage());
			throw ex;
		}
	}
	...
	public static String getGridCellValue(String gridName, String columnHeader, int moveFromIndex) throws Exception {
		...
		try {
			...
			jsCode = "return $('" + gridName + "').grid.cells($('" + gridName + "').grid.getSelectedRowId(),"
					+ columnIndex + ").getValue(); ";
			String value = (String) ((JavascriptExecutor) DriverFactory.getInstance().getDriver()).executeScript(jsCode);
			return value;

		} catch (Exception ex) {
			captureScreenShot(DriverFactory.getInstance().getDriver(), new Exception().getStackTrace()[0].getMethodName());
			Log.error(
					"Class WebdriverUtils | Method getGridCellValue | Exception occurred while executing javascript. Code: "
							+ jsCode + "   Exception: " + ex.getMessage());
			throw ex;
		}
	}
	...
	public static int getGridRowNumber(String gridName) throws Exception {
		...
		try {
			count = ((Long) ((JavascriptExecutor) DriverFactory.getInstance().getDriver()).executeScript(jsCode)).intValue();
		} catch (Exception ex) {
			captureScreenShot(DriverFactory.getInstance().getDriver(), new Exception().getStackTrace()[0].getMethodName());
			Log.error(
					"Class WebdriverUtils | Method getGridRowNumber | Exception occurred while executing javascript. Code: "
							+ jsCode + "   Exception: " + ex.getMessage());
			throw ex;
		}
		return count;
	...
	public static void selectGridRow(String gridName, int rowIndex) throws Exception {
		...
		try {
			...
			captureScreenShot(DriverFactory.getInstance().getDriver(), new Exception().getStackTrace()[0].getMethodName());
			Log.error(
					"Class WebdriverUtils | Method selectGridRow | Exception occurred while executing javascript. Code: "
							+ jsCode + "   Exception: " + ex.getMessage());
			throw ex;
		}
	}
	...
	public static boolean fluentWait(String Elem) throws Exception {
		...
		wait.until(new Function<WebDriver, WebElement>() 
		{
			public WebElement apply(WebDriver driver) {
				...
				try {
					WebElement element = DriverFactory.getInstance().getDriver().findElement(By.xpath(WebdriverUtils.findElementString(Elem)));
	...
	public static String findElemString(WebElement element) throws Exception {
		try {
			WebElement elementx=element;
			String str = elementx.toString().substring(element.toString().indexOf("xpath")).replace("xpath:", "").trim();
			String str1 = str.substring(0, str.length()-1);
	...
	public static boolean fluentWait(WebElement Elem) throws Exception {
		...
		wait.until(new Function<WebDriver, WebElement>() 
		{
			public WebElement apply(WebDriver driver) {
				System.out.println("Checking for the element!!");
				try {
					WebElement element = DriverFactory.getInstance().getDriver().findElement(By.xpath(WebdriverUtils.findElemString(Elem)));
					//WebElement element = Elem;
					if(element != null)
					{
						System.out.println("Target element found");
	...
	public static Boolean switchToNewTab(int i) throws Exception {
		try {
			//Get the list of window handles
			ArrayList<String> tabs = new ArrayList<String> ( DriverFactory.getInstance().getDriver().getWindowHandles());
			System.out.println(tabs.size());
			//Use the list of window handles to switch between windows
			DriverFactory.getInstance().getDriver().switchTo().window(tabs.get(i));
	...
	public static Boolean switchTodefaultTab() throws Exception {
		try {
			//Get the list of window handles
			ArrayList<String> tabs = new ArrayList<String> (DriverFactory.getInstance().getDriver().getWindowHandles());
			System.out.println(tabs.size());
