...
//		WebdriverUtils.waitUntilValuePresentInElement(PLPObjects.Quantity(pro.getProperty("ProductName")), "1");
		Thread.sleep(5000);
		if(PLPObjects.PLPFirstProductATC().isDisplayed()) {
		WebdriverUtils.clickButtonThroughJS(PLPObjects.PLPFirstProductATC(), "First available product");
		}
		else {
			WebdriverUtils.clickButtonThroughJS(PLPObjects.PLPFirstProductATCSale(), "First available sale product");
		}
		...
		if(FlavourpageObjects.productnameinpopup1().isDisplayed()) {
			...
			Assert.assertTrue(FlavourpageObjects.productnameinpopup1().isDisplayed());
